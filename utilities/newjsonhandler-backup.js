...
const var_json_path = path.resolve('./datajsons/varstorage.json');
...
function getCurrentTurn() {
	const rawData = fs.readFileSync(`${path.resolve('./datajsons/data.json')}`);
...
function getCharacterOwner(charid) {
	const jsonData = getCharacterJson(charid)
	return jsonData['data'].discordId
}
...
function getCharacterNamesAndIds() {
	const dirPath2 = path.resolve('./charactersheets/')
...
function doesCharacterExist(charid) {
	return fs.existsSync(`./charactersheets/${charid}.json`)
}
...
function updateCharacter(charid, interaction) {
	if (doesCharacterExist(charid)) {
		...
		request(options).then((response) => {
			...
			const rawData = fs.readFileSync(`${getCharacterSheetPath(charid)}`, 'utf-8');
			const jsonData = JSON.parse(rawData);
			interaction.reply(`${jsonData['data'].name} Updated`)
		}).catch((e) => {
			console.log(e)
		})
	} else {
...
function createNewCharacter(charid, playerid, interaction) {
	...
	request(options).then((response) => {
		...
		const rawData = fs.readFileSync(`${getCharacterSheetPath(charid)}`, 'utf-8');
		const jsonData = JSON.parse(rawData);
		interaction.reply(`${jsonData['data'].name} Created`).catch((e) => console.log('error'))
	}).catch((e) => {
		console.log(e)
	})
}
...
const createNewJson = (charid) => {
	...
	var isArtificer = 0, isBarbarian = 0, isBard = 0, isCleric = 0, isDruid = 0, isFighter = 0, isMonk = 0, isPaladin = 0, isRanger = 0, isRogue = 0, isSorcerer = 0, isWarlock = 0, isWizard = 0, isBloodHunter = 0, isDragonborn = 0, isDwarf = 0, isElf = 0, isHalfling = 0, isHalfOrc = 0, isHalfElf = 0, isHuman = 0, isTiefling = 0, isGnome = 0, isAarakocra = 0, isGenasi = 0, isGoliath = 0, isAasimar = 0, isBugbear = 0, isFirbolg = 0, isGoblin = 0, isHobgoblin = 0, isKenku = 0, isKobold = 0, isLizardfolk = 0, isOrc = 0, isTabaxi = 0, isTriton = 0, isYyantiPureblood = 0, isFeralTiefling = 0, isTortle = 0, isGith = 0, isChangling = 0, isKalashtar = 0, isShifter = 0, isWarforged = 0, isCentaur = 0, isLoxodon = 0, isMinotaur = 0, isSimicHybrid = 0, isVedalken = 0, levelBarbarian = 0;
	...
	var bardCollege = "", clericDomain = "", druidCircle = "", fighterArchetype = "", monkWay = "", paladinOath = "", rangerArchtype = "", rogueArchetype = "", sorcererOrigin = "", warlockPatron = "", wizardSchool = "", wearingArmor = 0;
	var usingHeavyArmor = 0, usingMediumArmor = 0, usingLightArmor = 0, usingShield = 0;
                ...
                current_class.subclassDefinition.classFeatures.some(function(findTotem, j) {
                    if(levelBarbarian >= findTotem.requiredLevel) {
                        if (findTotem.name.match("Totem Spirit")) {
                            ...
                            character.options.class.some(function(guessing, k) {
                                if (animalID == guessing.componentId) {
                                    let barbTotemAttune = guessing.definition.name;
                                }
                            });
                        }
                    }
...
module.exports = {
	...
	getCharacterJson: getCharacterJson,
